...
import { db, storage, doc, setDoc, ref, uploadBytes, getDownloadURL } from '../../firebaseConfig';
import { TextField, Button, Container, FormControl, InputLabel, Select, MenuItem, Box, Typography, Paper } from '@mui/material';
...
const AddEditPantryItem = ({ item, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (imageFile) {
        const imageRef = ref(storage, `pantryItems/${new Date().toISOString()}_${imageFile.name}`);
        await uploadBytes(imageRef, imageFile);
        imageUrl = await getDownloadURL(imageRef);
      }

      const itemRef = doc(db, 'pantryItems', item ? item.id : new Date().toISOString());
    ...
    >
      ...
      <Box sx={{ mb: 2 }}>
        <TextField
          ...
          sx={{ mb: 2 }}
        />
        <Box sx={{ display: 'flex', gap: 2 }}> {/* Added a Box to contain Quantity and Unit */}
          <TextField
            label="Quantity"
            value={quantity}
            ...
            sx={{ width: 300 , mb: 1}}
          />
          <FormControl sx={{ width: 100 }}> {/* Adjust width as needed */}
